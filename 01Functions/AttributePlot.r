...
#### CurrentFileName <- paste0(RawDataFolder,"/",FileList[7])
...
#### Function1 <- paste0(FolderRoute,"/01Functions/AttributePlot.r")
#### source(Function1)
#### AttributePlot(TICdata,modulation_time,CurrentFileName)


AttributePlot <- function(Attribute,modulation_time,filename){
...
Intensity[(i-1)*second_dimension_resolution+j] = Attribute_matrix_updown[j,i]

#1st dimention time index
GC1d[(i-1)*second_dimension_resolution+j] = first_dimension_axis[1,i]

#2nd dimention time index
GC2d[(i-1)*second_dimension_resolution+j] = second_dimension_axis[j,1]
