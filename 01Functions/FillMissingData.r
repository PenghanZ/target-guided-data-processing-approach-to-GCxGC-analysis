...
FillMissingData <- function(filename, modulation_time){
library(ncdf4)
#filename1 <- filename
...
FilledData <- array(0,dim = c(second_dimension_resolution, first_dimension_resolution, MzRangeLength), 
...
function5 <- paste0(FolderRoute,"/01Functions/AttributeMatrix.r")
source(function5)
...
SignalMatrix <- AttributeMatrix(MzData,modulation_time,filename)
FilledData[, ,which(MzRange == MzValue)] <- SignalMatrix
